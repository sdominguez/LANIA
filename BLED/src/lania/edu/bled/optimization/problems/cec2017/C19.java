 ...
 * @author sdominguez
 */
public class C19 extends ConstrainedProblem{
    ...
    public Solution evaluate(Solution s) {
        ...
        for (i = 0; i < nx; i++) {
            f1 = f1 + (Math.pow(Math.abs(e[i]), 0.5) + 2*Math.sin(Math.pow(e[i], 3)));
        }
        
        for (i = 0; i < (nx - 1); i++) {
            g1 = g1 + (-10*Math.exp(-0.2 * Math.sqrt(Math.pow(e[i], 2) + Math.pow(e[i+1], 2))));
        }
        
        for (i = 0; i < nx; i++) {
            g2 = g2 + (Math.pow(Math.sin(2*e[i]), 2));
        }
