 ...
 * @author sdominguez
 */
public class Rand2 extends Mutation {
    ...
    public List<Solution> getMutants(double[] f, List<Solution> targets, ConstrainedProblem problem) {
        ...
        for (int i = 0; i < popSize; i++) {
            ...
            int[] r = Stochastic.arrayRandInt(5, 0, popSize - 1, i);
            ...
            double[] x4 = targets.get(r[3]).getVariables().clone();
            double[] x5 = targets.get(r[4]).getVariables().clone();
            double[] x = targets.get(i).getVariables().clone();
            /*Aplicar operador de mutacion*/
            for (int k = 0; k < n; k++) {
                //mutant.X[j] = Boundary.Reflexion(t1.X[j] + (F * (t2.X[j] - t3.X[j])) + (F * (t4.X[j] - t5.X[j])));
                x[k] = x1[k] + (f[k] * (x2[k] - x3[k])) + (f[k] * (x4[k] - x5[k]));
            }
